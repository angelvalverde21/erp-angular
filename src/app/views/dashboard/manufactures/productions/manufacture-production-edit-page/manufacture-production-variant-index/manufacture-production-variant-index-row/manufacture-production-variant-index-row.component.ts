...
@Component({
  selector: 'tr[app-manufacture-production-variant-index-row]',
  imports: [
    FontAwesomeModule,
    ButtonComponent,
    ReactiveFormsModule,
  ...
  templateUrl: './manufacture-production-variant-index-row.component.html',
  styleUrl: './manufacture-production-variant-index-row.component.scss',
  encapsulation: ViewEncapsulation.None

})
export class ManufactureProductionVariantIndexRowComponent implements OnDestroy, OnInit {
  ...
  removeProductionVariant(manufacture_variant_id: number, production_id: number) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this._manufactureVariant.setManufactureId(production_id);

        this._manufactureVariant.destroy(manufacture_variant_id).pipe(takeUntil(this.destroy$)).subscribe({

          next: (resp: any) => {

            console.log(resp);
            this.removeLoading = false;
            this.emitDeleteProductionVariantId.emit(manufacture_variant_id);
  ...
  editProductionVariant(content: TemplateRef<any>, manufacture_variant_id: number, production_id: number) {
  ...
  update() {
    ...
    console.log(this.manufacture_variant.manufacture_id);
    

    this._manufactureVariant.setManufactureId(this.manufacture_variant.manufacture_id);

    this._manufactureVariant.update(this.manufacture_variant.id, this.form.value).pipe(takeUntil(this.destroy$)).subscribe({
