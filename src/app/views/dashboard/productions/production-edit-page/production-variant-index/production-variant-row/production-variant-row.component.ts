...
@Component({
  selector: 'tr[app-production-variant-row]',
  imports: [
    FontAwesomeModule,
    ButtonComponent,
    ReactiveFormsModule,
  ...
  templateUrl: './production-variant-row.component.html',
  styleUrl: './production-variant-row.component.scss',
  encapsulation: ViewEncapsulation.None

})
export class ProductionVariantRowComponent implements OnDestroy, OnInit {
  ...
  @Output() emitDeleteProductionVariantId = new EventEmitter<number>();
  ...
  @Input() production_variant: any = {};

  removeLoading: boolean = false;

  removeProductionVariant(production_variant_id: number, production_id: number) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this._productionVariant.setProductionId(production_id);

        this._productionVariant.destroy(production_variant_id).pipe(takeUntil(this.destroy$)).subscribe({

          next: (resp: any) => {

            console.log(resp);
            this.removeLoading = false;
            this.emitDeleteProductionVariantId.emit(production_variant_id);
  ...
  editProductionVariant(content: TemplateRef<any>, production_variant_id: number, production_id: number) {
  ...
  ngOnInit(): void {
    ...
    this.form.patchValue({
      quantity: this.production_variant.quantity,
      price: this.production_variant.price
    });
  ...
  update() {
    ...
    this._productionVariant.setProductionId(this.production_variant.production_id);

    this._productionVariant.update(this.production_variant.id, this.form.value).pipe(takeUntil(this.destroy$)).subscribe({

      next: (resp: any) => {
        console.log(resp);
        this.loading = false;
        this.emitUpdatedQuantity.emit(resp.data); //emite el production_variant actualizado
      },
