...
export class AttributeValueService{
  ...
  constructor(protected http: HttpClient, private _store: StoreService) {
  ...
  index(product_id: number): Observable<any[]> {
    const url = `${this.baseUrl}/products/${product_id}/attribute-values`;
    console.log(url);
    return this.http.get<any[]>(`${url}`);
  }

  // Generic method to get a single item by ID
  get(attribute_value_id: number): Observable<any> {
    const url = `${this.baseUrl}/${attribute_value_id}`;
    console.log(url);
  ...
  destroy(product_id: number, attribute_value_id: number): Observable<any> {
    const url = `${this.baseUrl}/products/${product_id}/attribute-values/${attribute_value_id}`;
    console.log(url);
  ...
  sort(product_id: number = 0, data: any): Observable<any> {
    const url = `${this.baseUrl}/products/${product_id}/attribute-values/sort`;
    console.log(url);
    return this.http.put(`${url}`, data);
  }
